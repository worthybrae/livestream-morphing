...
def create_segment_video(segment_number, output_path, crf=26, scale=0.7, fps=30):
    ...
    for i in range(180):  # 0-179 frames
        ...
        if os.path.exists(frame_path):
            ...
            if frame is not None:
                ...
                cv2.imwrite(str(temp_dir / f"{i:04d}.jpg"), resized_frame,
    ...
    cmd = [
        'ffmpeg',
        '-framerate', str(fps),
        '-i', str(temp_dir / '%04d.jpg'),
        '-c:v', 'libx265',        # H.265 codec for better compression
        '-crf', str(crf),         # Slightly better quality (lower CRF)
        '-preset', 'medium',      # Balanced compression/speed
        '-pix_fmt', 'yuv420p',    # Standard pixel format        # Optimize for film-like content
        temp_output
    ]
...
def combine_segments(segment_files, output_path):
    ...
    cmd = [
        ...
        '-safe', '0',
        '-i', 'segments.txt',
        '-c:v', 'libx265',      # Use H.265 for final output
        '-crf', '5',           # Better quality for final output
        '-preset', 'medium',    # Balance between compression and encoding speed      # Optimized for film-like content
        output_path
    ]
...
def main():
    ...
    segment_files = []
    for segment in range(1710, 1720):  # 9091 to 9094 segments
